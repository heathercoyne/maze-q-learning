...
class QLearningAgent:
    ...
    #  1-step fallback update (used when 2-step target is not available)
    #  Q(s,a) <- Q(s,a) + alpha * [ r + gamma*max_a' Q(s',a') - Q(s,a) ] 
    def learn_1step(self, s, a, r, s_, done):
        s_key = self._state_to_key(s)
        self._ensure_state(s_key)
    ...
    # 2 step Q learning, Update Q(s_t, a_t) using r_{t+1}, r_{t+2}, and s_{t+2}
    def learn_2step(self, s0, a0, r1, r2, s2, done2):
        s0_key = self._state_to_key(s0)
        self._ensure_state(s0_key)

        q_predict = self.q_table.loc[s0_key, a0]

        if done2:
            # episode ends at t+2: target = r1 + gamma*r2
            q_target = r1 + self.gamma * r2
        else:
            # normal: target = r1 + gamma*r2 + gamma^2 * max_a Q(s2, a)
            s2_key = self._state_to_key(s2)
            self._ensure_state(s2_key)
            q_target = r1 + self.gamma * r2 + (self.gamma ** 2) * self.q_table.loc[s2_key, :].max()

        self.q_table.loc[s0_key, a0] = q_predict + self.alpha * (q_target - q_predict)


def train_2step_q_learning(
    env,
    agent,
    episodes=800,
    max_steps_per_episode=200,
    render_every=80, # visualize every N episodes (chaned from 50 to 80)
    stable_window=60, # check last N episodes (changed from 50 to 60)
    ...
    for ep in range(1, episodes + 1):
        ...
        # pending holds one transition (s_t, a_t, r_{t+1}, s_{t+1})
        pending = None
        ...
        while (not done) and (steps < max_steps_per_episode):
            ...
            s1, r1, done1 = env.step(a)
            steps += 1

            # If have one transition pending can do a 2-step update 
            if pending is not None:
                s0, a0, r_prev, _s_next = pending  # _s_next is s_t
                agent.learn_2step(s0, a0, r_prev, r1, s1, done2=done1)

            # if current step ends the episode
            if done1:
                agent.learn_1step(s, a, r1, s1, done=True)
                done = True
                success = 1 if r1 == 1 else 0
                break

            # store current transition as pending for the next step
            pending = (s, a, r1, s1)
            s = s1

        # if hit max step but havent reached terminal
        if (not done) and (steps >= max_steps_per_episode) and (pending is not None):
            s0, a0, r1p, s1p = pending
            agent.learn_1step(s0, a0, r1p, s1p, done=False)
